...
export function answerFor(intentId, ctx) {
  ...
  switch (intentId) {
    ...
    case 'password':
      /* คำตอบเดียวกับกล่อง "ลืมรหัสผ่าน?" ในหน้าล็อกอิน (t.helpBody ใน LoginPage.jsx)
         ถ้าจะแก้ที่ทางเดินของคนขอรหัสใหม่ ต้องแก้ทั้งสองที่ให้ตรงกันเสมอ

         ไม่แยกตาม role แล้ว ทั้งที่ตรงนี้รู้ role อยู่ — ฝ่ายทะเบียนเป็นจุดรับเรื่องจุดเดียว
         ของทุกคน และของเดิมที่สั่งให้ "นำบัตรประจำตัวนักเรียน" มาด้วย
         ใช้กับครู/ฝ่ายวิชาการไม่ได้ จึงเหลือแค่ "ไปที่ไหน" ซึ่งตรงกับทุกบทบาท */
      return {
        text: 'รหัสผ่านตั้งใหม่เองไม่ได้ครับ (ระบบไม่เปิดให้ตั้งเองเพื่อความปลอดภัย)\nกรุณาติดต่อที่ฝ่ายทะเบียน ที่อาคาร 1 ชั้น 1 เพื่อขอรหัสผ่านใหม่',
      };
