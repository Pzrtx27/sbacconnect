...
export function answerFor(intentId, ctx) {
  ...
  switch (intentId) {
    ...
    case 'password':
      /* แยกตาม role ได้เต็มที่ตรงนี้ เพราะกว่าจะคุยกับผู้ช่วยได้ก็ล็อกอินผ่านแล้ว
         ต่างจากกล่อง "ลืมรหัสผ่าน?" ในหน้าล็อกอิน (t.helpBody ใน LoginPage.jsx)
         ที่ยังไม่รู้ว่าคนกดเป็นใคร จึงต้องใช้ข้อความกลาง

         ฝ่ายทะเบียน อาคาร 1 ชั้น 1 เป็นทางเดินของ "นักเรียน" เท่านั้น
         ครู/ฝ่ายวิชาการ/แอดมิน ไม่ได้ขอรหัสใหม่ที่นั่น — ของเดิมส่งทุกบทบาทไปที่เดียวกันหมด
         ทำให้บุคลากรถูกสั่งให้เดินไปผิดตึก

         ห้ามเติมห้อง/ตึก/เบอร์ของฝั่งบุคลากรลงไปเองจนกว่าจะยืนยันกับทางวิทยาลัย
         ข้อความที่ระบุสถานที่คือสิ่งที่คนเชื่อแล้วเดินไปตามนั้นจริง */
      return {
        text: isStudent
          ? 'รหัสผ่านตั้งใหม่เองไม่ได้ครับ (ระบบไม่เปิดให้ตั้งเองเพื่อความปลอดภัย)\nกรุณาติดต่อที่ฝ่ายทะเบียน ที่อาคาร 1 ชั้น 1 เพื่อขอรหัสผ่านใหม่'
          : 'รหัสผ่านตั้งใหม่เองไม่ได้ครับ (ระบบไม่เปิดให้ตั้งเองเพื่อความปลอดภัย)\nบัญชีบุคลากรต้องให้ผู้ดูแลระบบตั้งรหัสใหม่ให้',
      };
