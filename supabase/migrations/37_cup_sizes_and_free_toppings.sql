...
begin
  ...
  /* ยามกัน 38 ถูกย้อน — เพิ่มเข้ามาทีหลังตอนที่ 38_cup_oz_and_menu_realtime.sql
     เปลี่ยนชื่อขนาดให้มีจำนวนออนซ์ต่อท้าย (เล็ก 12 oz / กลาง 16 oz / ใหญ่ 22 oz)

     ไฟล์นี้เขียนไว้ว่า "รันซ้ำได้" ซึ่งชวนให้คนหยิบมารันเดี่ยว ๆ เวลาสงสัยว่าตัวเลือกหาย
     แต่ถ้ารันหลัง 38 ท่อน insert ข้างล่างจะเห็นว่าไม่มีแถวชื่อ 'แก้วเล็ก (S)' แล้ว
     เลยใส่กลับเข้าไปใหม่ กลายเป็นขนาดหกแบบพร้อมกัน (สามแบบเก่า + สามแบบใหม่)
     ตรวจก่อนว่าขนาดรุ่นออนซ์มาแล้วหรือยัง ถ้ามาแล้วให้ข้ามส่วนนี้ทั้งหมด */
  if g_size is not null and exists (
    select 1 from public.options
     where group_id = g_size and name = 'กลาง 16 oz'
  ) then
    raise notice 'ขนาดแก้วเป็นรุ่นออนซ์แล้ว (38_cup_oz_and_menu_realtime.sql) — ข้ามส่วนขนาด';
    g_size := null;
  end if;

  if g_size is null then
    raise notice 'ข้ามส่วนขนาดแก้ว (ยังไม่มีกลุ่ม "ขนาด" หรือถูกแทนที่ด้วยรุ่นออนซ์แล้ว)';
  else
